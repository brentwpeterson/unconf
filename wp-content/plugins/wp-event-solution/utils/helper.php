...
class Helper {
					...
					<select name="etn_event_location" class="etn_event_select2 etn_event_select">
						...
						if (is_array($location_data) && !empty($location_data)) {
							...
							foreach ($location_data as $value) {
								...
								?>
							<option <?php if (!empty($select_value) && $select_value === $value ) echo ' selected="selected"' ; ?> value="<?php echo esc_attr( $value ); ?>"><?php echo esc_html( $value ); ?></option>
								<?php
							}
						}
						?>
					...
					<select name="etn_categorys" class="etn_event_select2 etn_event_select">
						...
						if (!empty($category_data) && is_array($category_data)) {
							...
							foreach ($category_data as $key => $value) { ?>
								<option <?php if (!empty($select_cat_value) && $select_cat_value == $key ) echo ' selected="selected"' ; ?> value="<?php echo esc_attr( $key ); ?>"><?php echo esc_html( $value ); ?></option>
							<?php }
						}
						?>
					</select>
			...
			if (!empty($etn_event_date_range)) {
				if ($etn_event_date_range === "today") {
					...
					$meta_date_query = [
						...
						[
							'key'     => 'etn_start_date',
							'value'   => array($week_start,$weekend),
							'type'    => 'date',
							'compare' => 'BETWEEN'
						],
						[
    ...
    */
     ...
     * update attendee status and send ticket to email
     */
    public static function mail_attendee_report( $pdf_data ){
    ...
    public static function day_name(){
        return  array('Sun' => 'Sun','Mon' => 'Mon','Tue' => 'Tue',
