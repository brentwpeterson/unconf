...
class Hooks{
    ...
    public function init(){
        ...
        if( class_exists('WooCommerce') ){
            ...
            add_filter( 'woocommerce_cart_item_name', [$this,'child_event_permalink'], 10, 3 );
        }
    }

    public function child_event_permalink( $item_name , $cart_item,  $cart_item_key ){
        $content_post = get_post($cart_item['data']->get_id());$name ="";
        
        if ( get_post_type( $content_post ) !=="product" && ( is_cart() || is_checkout() )  && !empty( $content_post ) ) {
            $product_name = $cart_item['data']->get_title();
            $item_name_new = sprintf( '<a href="%s">%s</a>', esc_url( get_permalink( wp_get_post_parent_id($content_post) ) ), $product_name );
            $item_name_new =  $item_name_new;
            $name = "<div class='etn_recur_hide_child'>".$item_name_new ."<div>";
        }else {
            $name=$item_name;
        }

        echo Helper::kses( $name );
    }
    ...
    public function detach_child_event(){
        ...
        if ( is_admin()  && isset($_GET['action']) &&  'detach' == sanitize_text_field($_GET['action']) ) {
            if (! wp_verify_nonce( $_REQUEST['_wpnonce'], 'detach_nonce' ) ) {
                ...
                if ( !empty( $content_post) && 0 !== wp_get_post_parent_id($content_post)  ) {
    ...
    public function create_recurrences( $post_id, $post, $update ){
        ...
        if ( (true == $update) && ('etn' == get_post_type( $post)) && ( 0 == wp_get_post_parent_id($post) ) ) {
            ...
            $is_recurring_event = $recurring_events['is_recurring'] ? true : false;
    ...
    public function process_existing_recurrence_meta( $parent_post_id ){
        ...
        if( false !== $has_child_events && is_array( $has_child_events ) && !empty( $has_child_events ) ){
            ...
            foreach( $has_child_events as $child_event ){
                ...
                // generate new end date according to updated span days
